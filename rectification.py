...
def rectification(img1, img2, pts1, pts2, F, output_dir="."):
    """This function is used to rectify the images to make camera pose's parallel and thus make epiplines as horizontal.
        ...
        Args:
            output_dir: Directory where rectified images will be saved
    """
    ...
    # Rectify the images and save them to the specified output directory
    img1_rectified = cv2.warpPerspective(img1, H1, (w1, h1))
    img2_rectified = cv2.warpPerspective(img2, H2, (w2, h2))
    
    # Save to the organized output directory
    cv2.imwrite(os.path.join(output_dir, "rectified_1.png"), img1_rectified)
    cv2.imwrite(os.path.join(output_dir, "rectified_2.png"), img2_rectified)
